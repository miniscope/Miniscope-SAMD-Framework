...
static xdmac_channel_config_t xdmac_cfg[6]; //XDMAC channel configuration.
...
void systick_init(void) {
	...
	ms_ticks = 0;
	// Systick timer that performs timer/counter operations. This line configures the system clock
...
void spi_config_interface_mode(void) {
	...
	SPI_MASTER_BASE->SPI_CR = SPI_CR_SPIDIS; //Disabled SPI
	// sets the pin to SPI MISMOSI pinsO
	ioport_set_pin_mode(SPI_MOSI_PIN,SPI_MOSI_MODE);
	ioport_disable_pin(SPI_MOSI_PIN);
...
int main (void)
{
	...
	// sets state of the Naneye, this is a bitshift
	state = STATE_IDLE;
...
void XDMAC_Handler(void) {
	...
	if (dma_status & XDMAC_CIS_BIS) { //If SPI DMA is done
		...
		switch (state) {
			...
			if (naneye_new_reg_val_received) { // this me we just wrote new reg values to the naneye
